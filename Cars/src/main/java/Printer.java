...
public class Printer {
    ...
    public void print(final List<Integer> idList, final List<String> brandList, final List<String> modelList,
                      final List<Integer> ageOfProduceList) {

        System.out.println(STRING_SEPARATOR);
        for (Integer element : idList) {
            System.out.print(String.format(OUTPUT_PATTERN, element, brandList.get(index), modelList.get(index),
                    ageOfProduceList.get(index)));
            System.out.println(STRING_SEPARATOR);
            index++;
        }
    }
